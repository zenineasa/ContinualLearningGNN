...
int main(int argc, char* argv[])
{
    ...
    for (int timestep = 1; timestep <= nt; timestep++)
    {
        ...
        if (!converged) {
            std::cerr << "step " << timestep
                      << " ERROR : nonlinear iterations failed to converge - serial" << std::endl;;
            break;
        }

        {
            ...
            s = "output101/heateq_" + s + ".raw";

            FILE* output = fopen(s.c_str(), "w");
    ...
    std::ofstream myfile;
    myfile.open ("log.data", std::ios::app);
    myfile << "Threads: " << omp_get_max_threads() << "\tTime: " << timespent << std::endl;
    myfile.close();
